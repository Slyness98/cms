...
//BREAK IN PHP ECHO STATEMENT FOR A QUICK FETCH OF CATEGORY TITLE INFO WE'LL NEED TO GRAB FROM OUR DB FOR DROPDOWN MENU LINK GENERATION				                
...
while ($row = mysqli_fetch_assoc($select_all)) {
 ...
 $cat_id = $row['cat_id'];
			...
			echo " 				    <li>
				              			 <a 
				              			 	class='dropdown-item' 
				              				href='../cmsTest/categories/{$cat_id}'> 
				              						{$cat_title} 
				              			 </a> 
		                      	    </li>";
}//END WHILE LOOP FOR CATEGORY DROPDOWN. RESUME NAV COMPONENT ECHO.
...
if(isset($_SESSION['role']) && isset($_GET['p_id'])) {
	...
	echo "  </ul> ";//End left-aligned nav-items
...
if(!$_SESSION['isLoggedIn']){ 	
	...
	echo "          <ul class='nav navbar-right top-nav'>
				        <li class='nav-item dropdown text-center'>
			                    <a href='#' class='dropdown-toggle nav-link' data-toggle='dropdown'>Your Account<br><i class='fa fa-user'></i> $username <b class='caret'></b></a>
			                    <ul class='dropdown-menu'>
			                        <li class='dropdown-item'>
			                            <a class='nav-link' href='../profileSubscriber.php'><i class='fa fa-fw fa-user'></i> Profile</a>
			                        </li>
